...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    const Embed = new Discord.MessageEmbed()
      .setColor(Color)
      .setTitle("Success")
      .setDescription(`<:nice:787706440801321010> 24/7 Has Been ${Oned.Always === false ? "Enabled" : "Disabled"}`)
      .setTimestamp();
    
    Oned.Always = Oned.Always === false ? true : false;

    return message.channel.send(Embed).catch(() => message.channel.send(`<:nice:787706440801321010> 24/7 Has Been ${Oned.Always === false ? "Enabled" : "Disabled"}`));
