...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    .setDescription(`:infinity:  Loop Status - ${Queue.Loop ? "On" : "Off"}`)
    .setTimestamp();
    ...
    if (args[0] === "on") {
      Queue.Loop = true;
      Embeded.setDescription(":infinity:  Loop Has Been Enabled!")
      return message.channel.send(Embeded).catch(() => message.channel.send("Loop Has Been Enabled!"))
    } else {
      Queue.Loop = false;
      Embeded.setDescription(":infinity:  Loop Has Been Disabled!");
